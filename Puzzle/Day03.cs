...
namespace AdventOfCode.Puzzle
{
    public class calc
    {
        ...
        public calc(int i, int j, int score, bool final)
        {
            this.i = i;
            this.j = j;
            this.score = score;
    ...
    internal class Day03 : IDayPuzzle
    {
        public string ExecutePart1()
        {
            ...
            {
                bool tocheck = false;
                int indexI = 0, indexJ = 0, length = 0, number = 0;
                ...
                for(int j = 0; j < input[i].Length; j++)
                {
                    if (char.IsNumber(input[i][j]))
                    {
                        ...
                        length++;
                        number = number * 10 + int.Parse(input[i][j].ToString());
                    }
                    
                    if (!char.IsNumber(input[i][j]) || j == input[i].Length - 1)
                    {
                        if (tocheck)
                        {
                            bool symbol_around = checkForSymbolsAroundpart1(indexI, indexJ, length);
                            if (symbol_around) sum_parts += number;
            ...
            return sum_parts.ToString();
        }

        private bool checkForSymbolsAroundpart1(int index_i, int index_j, int length)
        {
            for(int i = index_i - 1; i < index_i + 2; i++)
            {
                if (i > -1 && i < input.Length)
                {
                    for (int j = index_j - 1; j < index_j + length+1; j++)
                    {
                        if (j > -1 && j < input[0].Length)
                        {
        ...
        public string ExecutePart2()
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                for (int j = 0; j < input[i].Length; j++)
                {
                    ...
                    if (!char.IsNumber(input[i][j]) || j == input[i].Length - 1)
                    {
                        ...
                        {
                            int i_loc, j_loc;
                            bool symbol_around = checkForSymbolsAround(indexI, indexJ, length, out i_loc, out j_loc);
                            if (symbol_around)
                            {
                                calc calc = new calc(i_loc, j_loc, number, false);
                                calc found = calcs.FirstOrDefault(c => c.i == calc.i && c.j == calc.j);
                                if (found == null) calcs.Add(calc);
                                else
                                {
                                    found.final = true;
                                    found.score *= number;
            ...
            return calcs.Where(c => c.final).Sum(c => c.score).ToString();
        }

        private bool checkForSymbolsAround(int index_i, int index_j, int length, out int loc_i, out int loc_j)
        {
            for (int i = index_i - 1; i < index_i + 2; i++)
            {
                if (i > -1 && i < input.Length)
