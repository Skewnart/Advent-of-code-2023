...
{
    internal class Day01 : IDayPuzzle
    {
        public List<string> words = new List<string>(){ "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
        
        public string ExecutePart1()
        {
            //Console.WriteLine(string.Join(", ", input.Split("\n").Select(x => int.Parse($"{x.First(y => char.IsNumber(y))}{x.Last(y => char.IsNumber(y))}"))));
            return input.Split("\n").Select(x => int.Parse($"{x.First(y => char.IsNumber(y))}{x.Last(y => char.IsNumber(y))}")).Sum().ToString();
        }
        public string ExecutePart2()
        {
            ...
            foreach (string part in parts)
            {
                ...
                for (int i = 0; i < part.Length; i++)
                {
                    ...
                    string sub = part.Substring(i);
                    foreach (string word in words)
                    {
                        if (sub.StartsWith(word))
                        {
                            res += (words.IndexOf(word) + 1).ToString();
                            break;
                        }
                    }
                ...
                for (int i = part.Length - 1; i >= 0; i--)
                {
                    ...
                    string sub = part.Substring(0, i + 1);
                    foreach (string word in words)
                    {
