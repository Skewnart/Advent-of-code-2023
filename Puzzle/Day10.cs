...
namespace AdventOfCode.Puzzle
{
    ...
    public class Agent
    {
        ...
        public Agent(int x, int y, int dx, int dy, int value)
        {
        ...
        public void Move()
        {
            char letter = Day10.input[this.Position.X][this.Position.Y];
            if (letter == 'J')
            {
                if (this.dX == 0 && dY == 1)
                {
    ...
    public class Day10 : IDayPuzzle
    {
        Point S = new Point(90, 62);
        Agent A1 = new Agent(89, 62, -1, 0, 1);
        Agent A2 = new Agent(90, 63, 0, 1, 1);

        Dictionary<Point, int> points = new Dictionary<Point, int>();

        public string ExecutePart1()
        {
            while(!points.Keys.Any(x => x.Equals(A1.Position) || x.Equals(A2.Position)))
            {
                points.Add(new Point(A1.Position), A1.Value);
                points.Add(new Point(A2.Position), A2.Value);

                A1.Move();
                A2.Move();
            }

            return points.MaxBy(x => x.Value).Value.ToString();
        }
